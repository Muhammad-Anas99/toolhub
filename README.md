...
| `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET` | Optional — enables "Continue with Google". Full Google Cloud Console setup steps are in `server/README.md`'s "Google OAuth" section. The redirect URI you register there must include this exact backend's domain. |
...
- **Authentication** — register, log in, log out, JWT access + refresh tokens (rotation, httpOnly cookie), forgot/reset password, email verification, and **Google OAuth** ("Continue with Google" — full setup instructions in `server/README.md`'s "Google OAuth" section). Full details in `server/README.md`'s "Authentication" section.
- **User dashboard** — Overview, Profile (edit name/email, upload/replace/remove a real profile picture), Favorites, Conversion History, Settings (change password, resend verification), Subscription (Free/Premium/Pro display, no payment yet). All under `/dashboard`, protected by `ProtectedRoute`. There is deliberately no Downloads section — it was built and then removed; History remains the single record of tools used.
- **Favorites & history wired into the tools themselves** — every tool page has a Favorite button (`FavoriteButton` in `ToolHeader`), and every successful conversion is logged automatically via `useToolResult` (works for signed-out users too, for site-wide analytics, without a personal history entry). The actual file download (`download()` in that same hook) is a pure local browser action — nothing about it is persisted server-side.
...
- **Navbar** — a single "Get Started" button (visible directly on mobile, not hidden in the hamburger menu) replaces separate Log In/Create Account links when signed out; the account avatar menu replaces it once signed in. "Image Tools" and "PDF Tools" have their own hover dropdowns (capped at 6 items, no scrollbar, "View all" for the rest) alongside the full "Categories" mega menu.
