...
import { HiOutlineArrowDownTray, HiOutlineSpeakerXMark } from 'react-icons/hi2'
...
export default function VideoMuterTool({ toolSlug, toolName, category }) {
  ...
  async function handleMute() {
    ...
    try {
      const blob = await reencodeVideo(upload.file, { keepAudio: false, onProgress: setProgress })
      setResult(blob)
      setStatus('done')
      logNow('Video muted')
    } catch (err) {
  ...
  function handleDownload() {
    if (!result) return
    downloadBlob(result, buildOutputFilename(upload.file.name, 'webm', '-muted'))
  }

  return (
    <div className="space-y-5">
      <div className="rounded-xl bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:bg-amber-950 dark:text-amber-300">
        This plays through the video to re-record it without audio, so it takes about as long as the video itself.
      ...
      {!upload.file ? (
        ...
        <>
          ...
          {status === 'idle' && (
            <button type="button" onClick={handleMute} className="btn-primary w-full sm:w-auto">
