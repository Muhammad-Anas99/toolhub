...
import { HiOutlineArrowDownTray, HiOutlineArchiveBox } from 'react-icons/hi2'
...
const QUALITY_PRESETS = {
  light: { label: 'Light (smaller, minor quality loss)', scale: 75, bitsPerSecond: 2_000_000 },
  medium: { label: 'Medium (balanced)', scale: 50, bitsPerSecond: 1_000_000 },
  aggressive: { label: 'Aggressive (smallest file)', scale: 35, bitsPerSecond: 500_000 },
}

export default function VideoCompressorTool({ toolSlug, toolName, category }) {
  ...
  function handleDownload() {
    if (!result) return
    downloadBlob(result, buildOutputFilename(upload.file.name, 'webm', '-compressed'))
  }

  return (
    <div className="space-y-5">
      <div className="rounded-xl bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:bg-amber-950 dark:text-amber-300">
        This plays through the video to re-record it smaller, so it takes about as long as the video itself. Keep
        this tab active while it runs. The result downloads as WebM.
      </div>
        ...
        <>
          ...
          {dimensions && status === 'idle' && (
            <>
              ...
              <button type="button" onClick={handleCompress} className="btn-primary w-full sm:w-auto">
            ...
            <div className="flex items-center gap-3 rounded-xl bg-emerald-50 p-4 dark:bg-emerald-950">
              <div className="min-w-0 flex-1">
                <p className="text-sm font-medium text-emerald-800 dark:text-emerald-300">
                  {formatBytes(result.size)} (was {formatBytes(upload.file.size)})
                </p>
              </div>
