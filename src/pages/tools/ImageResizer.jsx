...
export default function ImageResizer() {
  ...
  function seedDimensions(dimensions) {
  ...
  if (upload.dimensions && width === '' && height === '') {
    seedDimensions(upload.dimensions)
  }
  ...
  async function handleResize() {
    ...
    try {
      const { blob, width: outWidth, height: outHeight } = await resizeImage(upload.file, {
        width: target.width,
      ...
      upload.setError(error.message || 'Something went wrong while resizing this image.')
      setStatus('idle')
    }
  }
  ...
  function handleDownload() {
    if (!result) return
    downloadBlob(result.blob, buildOutputFilename(upload.file.name, upload.file.name.split('.').pop(), '-resized'))
  }
